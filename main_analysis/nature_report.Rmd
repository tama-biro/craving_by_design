...
kable(s_5_multi, align = 'c',
      caption = 'Test 5 with previous choice and reward history.') %>%
  kableExtra::footnote(general = "*p < .05, **p < .01, ***p < .001") %>%
  kable_styling(latex_options = "HOLD_position")
```
...
out <- boxcox(data_dists$betting_rate + 1, lambda = seq(-25, 25, by = 0.25))
bc_dist_lambda <- out$lambda[which.max(out$objective)]

data_dists$betting_rate_bc <- boxcoxTransform(data_dists$betting_rate + 1,
                                              lambda = bc_dist_lambda)

cap_data_dist <- paste0('Distributions of Box-Cox corrected (lambda = ', bc_dist_lambda,  ') betting rates in blue/yellow background sessions for control/test treatment.')
```
...
ggplot(data_dists, aes(x = treatment, y = betting_rate_bc, fill = block_type)) +
  geom_boxplot() +
  scale_x_discrete(name = '',
                   breaks = c('control', 'test'),
                   labels = c('Control', 'Test')) +
  scale_y_continuous(name = 'Betting rate (Box-Cox transformed)') +
