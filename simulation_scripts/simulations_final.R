...
simulate_choice_vk2 <- function(sim_data) {
  ...
  for(i in 1:nrow(sim_data)) {
    ...
    if(back_k > 0 & sim_data$craver_x[i]) {
...
for(k in 1:5) {
  test_parallel <- mclapply(1:3, parallel_simulation, mc.cores = 4)
  
  test_parallel <- do.call(rbind, test_parallel) %>%
    as.data.frame
  
  write.csv(test_parallel, paste0('test_parallel220815_55_', k ,'.csv'), row.names = FALSE)
}
...
sim_data <- sim_data %>%
  mutate(ID = rep(1:N, each = 600),
         treat = rep(c(rep('test', 600), 
                       rep('control', 600)), 
                     each = N/2),
         craver_x = rep(c(rep(rep(0:1, each = 600), (N/4)*0.8), rep(1, 600*(N/2)*0.2)), 2))
...
sim_data <- sim_data %>%
  mutate(ID = rep(1:N, each = 600),
         treat = rep(c(rep('test', 600), 
                       rep('control', 600)), 
                     each = N/2),
         craver_x = rep(c(rep(rep(0:1, each = 600), (N/4)*0.4), rep(1, 600*(N/2)*0.6)), 2))
...
for(k in 1:5) {
  test_parallel <- mclapply(1:10, parallel_simulation, mc.cores = 4)
  
  test_parallel <- do.call(rbind, test_parallel) %>%
    as.data.frame
  
  write.csv(test_parallel, paste0('test_parallel220817_28_', k ,'.csv'), row.names = FALSE)
}
