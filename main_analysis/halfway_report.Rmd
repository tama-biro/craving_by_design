...
glmer_report <- function (mod) {
  n <- as.numeric(summary(mod)$devcomp$dims[1])
  r2_out <- round(r2_nakagawa(mod)$R2_conditional, 3)
...
```{r unc_and_treat, echo=FALSE, results=FALSE, message=FALSE, warning=FALSE}
data_plot_u <- data %>%
...
data_plot_t <- data %>%
  ...
  summarize(se = se(betting_rate),
            betting_rate = mean(betting_rate)) %>%
  ungroup %>%
  mutate(x_ax = treatment) %>%
  select(-treatment)

data_plot <- data_plot_u %>%
  rbind(data_plot_t) %>%
  mutate(type = rep(c('Uncertainty', 'Treatment'),
                    times = c(nrow(data_plot_u),
                              nrow(data_plot_t))),
         type = factor(type,
                       levels = c('Uncertainty', 'Treatment'),
                       labels = c("(A) Uncertainty", "(B) Treatment")),
         x_ax = factor(x_ax, levels = c("Low", "High", 'control', 'test'),
                       labels = c("Low", "High", 'Control', 'Test')))
...
t_5_unc <- t.test(betting_rate ~ aaron_mood, data = data_t_5_unc,
                alternative = 'greater')
t_5_unc_rep <- t_test_report(t_5_unc)
p_unc <- t_5_unc$p.value
p_unc <- if_else(p_unc < .001, 'p < .001',
                 if_else(p_unc < .01, 'p < .01',
                         if_else(p_unc < .05, 'p < .05',
                                 paste0('p = ', round(p_unc, 3)))))

# Test for treatment
data_t_5_treat <- data_5 %>%
  group_by(id, treatment) %>%
  summarize(betting_rate = mean(choice)) %>%
  ungroup

t_5_treat <- t.test(betting_rate ~ treatment, data = data_t_5_treat,
              alternative = 'less')
t_5_treat_rep <- t_test_report(t_5_treat)
p_treat <- t_5_treat$p.value
p_treat <- if_else(p_treat < .001, 'p < .001',
                 if_else(p_treat < .01, 'p < .01',
                         if_else(p_treat < .05, 'p < .05',
                                 paste0('p = ', round(p_treat, 3)))))

dat_text <- data.frame(
  label = c(p_unc, p_treat),
  type = c('(A) Uncertainty', '(B) Treatment'),
  x = c(1.5, 1.5),
  y = c(0.28, 0.28)
)
```

```{r unc_and_treat_plot, echo=FALSE, results=FALSE, message=FALSE, warning=FALSE, fig.height=5, fig.width=8, fig.align='center', include=TRUE, fig.cap='(A) Average betting rate in low and high uncertainty yellow sessions for cravers. (B) Average betting rate in yellow sessions for cravers in control and test treatment.'}
ggplot(data_plot, aes(x = x_ax, y = betting_rate)) +
...
aic_3 <- round(AIC(log_mod_3_prev_choice, log_mod_3_prev_trial), 0)
aic_3 <- c('AIC', as.character(aic_3$AIC))
bic_3 <- round(BIC(log_mod_3_prev_choice, log_mod_3_prev_trial), 0)
bic_3 <- c('BIC', as.character(bic_3$BIC))

s_trial_outcome <- s_3_prev_choice %>%
  right_join(s_3_prev_trial) %>%
  replace_na(list(' ' = '-',
                  '(1)' = '-',
                  '(2)' = '-')) %>%
  rbind(rep('-', 3)) %>%
