...
data_plot_95 <- data_plot_b_95 %>%
  rbind(data_plot_y_95) %>%
  mutate(session_color = rep(c('Blue', 'Yellow'),
                             times = c(nrow(data_plot_b),
                                       nrow(data_plot_y))),
         session_color = factor(session_color,
...
data_plot_100 <- data_plot_b_100 %>%
  rbind(data_plot_y_100) %>%
  mutate(session_color = rep(c('Blue', 'Yellow'),
                             times = c(nrow(data_plot_b),
                                       nrow(data_plot_y))),
         session_color = factor(session_color,
...
In order to check for the effect of previous trials, the effect of previous trial outcome was included and compared to the effect of the previous choice variable. Previous trial outcome works similarly to previous choice, but when trial is not played (due to Aaron being cheeky, i.e. high uncertainty), the trial is coded as a skip rather than play.

### Comparison of previous choice and previous trial outcome for model 2 - table 6
...
colnames(s_3_prev_choice) <- c('Variable', '(1)')
colnames(s_3_prev_trial) <- c('Variable', '(2)')

aov_3 <- anova(log_mod_3_prev_choice, log_mod_3_prev_trial)
aov_3 <- aov_chi_report(aov_3)
...
kable(s_trial_outcome, format = "latex", align = 'c',
      caption = 'Model 2 with previous choice (1) and previous trial outcome') %>%
  kableExtra::footnote(general = "*p < .05, **p < .01, ***p < .001") %>%
  kable_styling(latex_options = "HOLD_position")
```
...
### Test 3 model (includes test 2) - table 7
