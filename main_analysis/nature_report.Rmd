...
Paired t-test using Box-Cox transformed data showed higher betting rate in high reward sessions (`r t_test_1`, Cohen's D = `r effect_size_t_1`). Non-parametric paired Wilcoxon test using non-transformed data showed qualitatively same results (`r wilcox_1`).
...
log_mod_2 <- bglmer(choice ~ reward_value + uncertainty +
                      ...
                      major + sequence_number + reaction_time + (1 | id),
...
Test 3 is the same model with reward exposure added as a variable.
...
kable(s_23, align = 'c', caption = 'Logistic mixed model predicting betting (0: skip, 1: bet) for all participants in all trials. Variables are reward value (0: low, 1: high), uncertainty (0: low, 1: high), treatment (0: control, 1: test), session color (0: blue, 1: yellow), previous choice (0: skip, 1: bet), and the interaction between session color and treatment. Controls not shown were age, gender, major, and sequence number. Model 1 (1) is the model described and model 2 (2) is the model with reward exposure added.') %>%
  kableExtra::footnote(general = "*p < .05, **p < .01, ***p < .001") %>%
  kable_styling(latex_options = "HOLD_position")
```

Next we show the VIF for the model (2) with previous choice and reward exposure. VIF at 1 suggests there is no multicollinearity and VIF >10 suggests problematic collinearity.

```{r log_mod_3_multi_vif, echo=FALSE, results=FALSE, message=FALSE, warning=FALSE}
...
```{r log_mod_3_vif_out, echo=FALSE}
kable(vif_mod_3_out, align = 'c',
      caption = 'VIF for variables in test 3.') %>%
  kable_styling(latex_options = "HOLD_position")
```
...
s_3_5 <- glmer_report(log_mod_3_5)
```
...
accuracy_cravers <- data %>%
  filter(craver_2 == 1) %>%
  group_by(id) %>%
  slice_head(n = 1) %>%
  mutate(odds_1_correct = if_else(odds_guess_one_reply == 2, 1, 0),
         odds_2_correct = if_else(is.na(odds_guess_two_reply), 1,
                                  if_else(odds_guess_two_reply %in% c(2, 4), 1, 0)),
         odds_3_correct = if_else(is.na(odds_guess_three_reply), 1,
                                  if_else(odds_guess_three_reply %in% c(2, 4), 1, 0)),
         accuracy = mean(c(odds_1_correct, odds_2_correct, odds_3_correct))) %>%
  ungroup %>%
  select(id, accuracy)

accuracy_data <- accuracy_other %>%
  rbind(accuracy_cravers)
...
data_4 <- data %>%
  ...
  right_join(accuracy_data, by = "id")

lin_mod_4 <- lm(betting_rate ~ treatment + age + gender + major + accuracy,
                data = data_4)
...
log_mod_4 <- glm(craver ~ treatment + age + gender + major + accuracy,
...
Linear model showed following F-statistic: `r f_s4_lin`.
...
kable(s_5, align = 'c', caption = 'Logistic mixed effects model predicting betting in yellow background sessions for cravers (bet at least twice in yellow). Independent variables were reward value (0: low, 1: high), uncertainty (0: low, 1: high), treatment (0: control, 1: test), and previous choice (0: skip, 1: bet), and the interaction between uncertainty and treatment. Controls not shown were age, gender, major, and sequence number.') %>%
  kableExtra::footnote(general = "*p < .05, **p < .01, ***p < .001") %>%
  kable_styling(latex_options = "HOLD_position")
```

## Test 5 with reward exposure and previous choice
...
log_mod_5_multi <- bglmer(choice ~ reward_value + uncertainty * treatment +
                            age + gender + major + sequence_number +
                            previous_choice + reward_exposure + (1 | id),
                          data = data_5_multi, fixef.prior = t,
                          family = binomial(link = "logit"))

s_5_multi <- glmer_report(log_mod_5_multi)

s_5_multi_r <- c('Nakagawa R^2', round(r2_nakagawa(log_mod_5_multi)$R2_conditional, 3))
s_5_multi_n <- c('N', summary(log_mod_5_multi)$devcomp$dims[1])
...
kable(s_5_multi, align = 'c',
      caption = 'Test 5 with previous choice and reward exposure.') %>%
  kableExtra::footnote(general = "*p < .05, **p < .01, ***p < .001") %>%
  kable_styling(latex_options = "HOLD_position")
```

Next we show the VIF for the model with previous choice and reward exposure. VIF at 1 suggests there is no multicollinearity and VIF >10 suggests problematic collinearity.

```{r log_mod_5_multi_vif, echo=FALSE, results=FALSE, message=FALSE, warning=FALSE}
data_5_multi_vif <- data_5_multi[,c('reward_value', 'uncertainty', 'treatment',
                                    'age', 'gender', 'major',
                                    'sequence_number', 'previous_choice',
                                    'reward_exposure')] %>%
  mutate(reward_value = as.numeric(reward_value),
         ...
         gender = as.numeric(gender),
         major = as.numeric(major))
vif_mod_5_out <- usdm::vif(data_5_multi_vif)
```

```{r log_mod_5_vif_out, echo=FALSE}
kable(vif_mod_5_out, align = 'c',
      caption = 'VIF for variables in test 5 with reward exposure included.') %>%
  kable_styling(latex_options = "HOLD_position")
```
...
Paired t-test using Box-Cox transformed data showed higher betting rate in high uncertainty sessions (`r t_test_6_out`, Cohen's D = `r effect_size_t_6`). Non-parametric paired Wilcoxon test using non-transformed data showed qualitatively same results (`r wilcox_6`).
...
cap_unc_all <- paste0("Average betting rate in yellow backround sessions for low and high uncertainty sessions for all participants ", "(", t_test_6_out, ", Cohen's D = ", effect_size_t_6, ").")
```
...
The t-test investigating whether betting rate in yellow in test was different from 0 was significant at, `r t_test_7_1_out`, Cohen's D = `r effect_size_t_7_1`.

The t-test comparing betting in yellow between the control and test treatment was also significant, showing that betting rate was on average higher in test compared to control treatment (`r t_test_7_2_out`, Cohen's D = `r effect_size_t_7_2`). This test was run on Box-Cox transformed data with lambda = `r bc_72_lambda`. Non-parametric paired Wilcoxon test using non-transformed data showed qualitatively same results (`r wilcox_7_2`).
...
cap_treat_all <- paste0("Average betting rate in yellow sessions for control and test treatment ", "(", t_test_7_2_out, ", Cohen's D = ", effect_size_t_7_2, ").")
```
...
```{r betting_dist_plot_setup, echo=FALSE, results=FALSE, message=FALSE, warning=FALSE}
data_dists <- data %>%
  group_by(id, treatment, block_type) %>%
  summarize(betting_rate = mean(choice, na.rm=TRUE)) %>%
  ungroup

cap_data_dist <- 'Distributions of betting rates in blue/yellow background sessions for control/test treatment.'
```

```{r betting_dist_plot_out, echo=FALSE, results=FALSE, message=FALSE, warning=FALSE, fig.height=5, fig.width=8, fig.align='center', include=TRUE, fig.cap=cap_data_dist}
